...
main (void)
{
  ...
  read_dummy = palloc_get_page (PAL_ZERO);
  block_buffer = malloc(BLOCK_SECTOR_SIZE);
  ...
  // uint32_t *test = (uint32_t *) palloc_get_page(PAL_ZERO);
  // *test = 0x123456;
  // int idx = insert_swap_table(test);
  // *test = 0x654321;
  // int idx2 = insert_swap_table(test);
  // uint32_t *test2 = (char *) palloc_get_page(PAL_ZERO);
  // fetch_swap_table(idx, test2);
  // printf("STRING: %x\n", *test2);
  // fetch_swap_table(idx2, test2);
  // printf("STRING2: %x\n", *test2);
  ...
  palloc_free_multiple(corpse_array, 2);
